#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>

#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64

// Replace the following line with the actual logo data from the converter
extern const unsigned char my_logo[] PROGMEM = {
  
};

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1);

int verticalPosition = 0;
const int verticalStep = 5;
bool whiteScreenActive = false;

void setup() {
    Serial.begin(9600);

    if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
        Serial.println(F("SSD1306 allocation failed"));
        delay(500);
        for(;;);
    }

    display.clearDisplay();
    randomSeed(analogRead(A0));
}

void loop() {
    unsigned long startTime = micros();

    if (whiteScreenActive) {
        // White screen is active, do nothing (it will stay white)
    } else {
        // Check for the rare event
        if (random(670104) == 0) {
            // Rare event: Activate white screen
            whiteScreenActive = true;
            display.fillScreen(1);
        } else {
            // Normal operation: Draw the logo, clear the previous position, and update the vertical position
            display.drawBitmap(0, verticalPosition, my_logo, SCREEN_WIDTH, SCREEN_HEIGHT, 1);
            display.fillRect(0, verticalPosition, SCREEN_WIDTH, SCREEN_HEIGHT, 0);
            verticalPosition += random(-verticalStep, verticalStep + 1);
            verticalPosition = constrain(verticalPosition, 0, display.height() - SCREEN_HEIGHT);
        }
    }

    display.display();

    unsigned long endTime = micros();
    unsigned long iterationTime = endTime - startTime;

    // Display iteration time as a clock
    int minutes = iterationTime / 60000000;  // Convert microseconds to minutes
    int seconds = (iterationTime % 60000000) / 1000000;  // Convert remaining microseconds to seconds

    // Print the clock information at a specific position on the screen
    display.setCursor(0, 0);
    display.setTextColor(SSD1306_WHITE);
    display.setTextSize(1);
    display.print("Time: ");
    display.print(minutes);
    display.print("m ");
    display.print(seconds);
    display.print("s");

    // Optional: Clear the clock area on the screen before updating
    // display.fillRect(0, 0, SCREEN_WIDTH, 10, 0);

    // Optional delay to control the loop speed
    delay(500);
}
