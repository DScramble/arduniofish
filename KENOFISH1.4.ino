#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>

#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64

// Replace the following line with the actual logo data from the converter
const unsigned char my_logo[] PROGMEM = {
  
}
; const byte digitPatterns[10][10] = {
  { 0x00, 0x3E, 0x5B, 0x4F, 0x4F, 0x4F, 0x4F, 0x5B, 0x3E, 0x00 }, // 0
  { 0x00, 0x04, 0x0C, 0x14, 0x24, 0x04, 0x04, 0x04, 0x3E, 0x00 }, // 1
  { 0x00, 0x1E, 0x23, 0x21, 0x02, 0x04, 0x08, 0x10, 0x3F, 0x00 }, // 2
  { 0x00, 0x1E, 0x33, 0x20, 0x18, 0x04, 0x02, 0x23, 0x1E, 0x00 }, // 3
  { 0x00, 0x08, 0x18, 0x28, 0x48, 0xFF, 0x08, 0x08, 0x08, 0x00 }, // 4
  { 0x00, 0x3F, 0x20, 0x20, 0x1E, 0x01, 0x01, 0x62, 0x3C, 0x00 }, // 5
  { 0x00, 0x0E, 0x11, 0x20, 0x3C, 0x22, 0x21, 0x11, 0x0E, 0x00 }, // 6
  { 0x00, 0x3F, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x20, 0x00 }, // 7
  { 0x00, 0x0E, 0x11, 0x21, 0x0E, 0x11, 0x21, 0x11, 0x0E, 0x00 }, // 8
  { 0x00, 0x1E, 0x21, 0x21, 0x1F, 0x01, 0x02, 0x04, 0x78, 0x00 }  // 9
};



Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1);

int verticalPosition = 0;
const int verticalStep = 5;

unsigned long loopCount = 0;

void setup() {
    Serial.begin(9600);

    if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
        Serial.println(F("SSD1306 allocation failed"));
        delay(2000);
        for(;;);
    }

    display.clearDisplay();
    randomSeed(analogRead(A0));  // Seed the random number generator with an analog pin value
}

void loop() {
    // Rare event: fully white screen
    if (random(80537376367104) == 0) {
        display.fillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 1);
        display.display();
        delay(2000);  // Display for 2 seconds
        display.clearDisplay();
    } else {
        // Display the logo
        display.drawBitmap(0, verticalPosition, my_logo, SCREEN_WIDTH, SCREEN_HEIGHT, 1);

        // Display loop count as clock moving down the screen
        display.setTextSize(1);
        display.setTextColor(1);
        display.setCursor(110, verticalPosition);
        display.print(loopCount);

        display.display();

        // Clear the previous position of the image and clock
        display.fillRect(0, verticalPosition, SCREEN_WIDTH, SCREEN_HEIGHT, 0);
        display.fillRect(110, verticalPosition, SCREEN_WIDTH - 110, 8, 0);

        // Move the image up or down by a random value between -5 and 5 pixels
        verticalPosition += random(-verticalStep, verticalStep + 1);

        // Ensure the image stays within the display bounds
        verticalPosition = constrain(verticalPosition, 0, display.height() - SCREEN_HEIGHT);

        // Increment loop count
        loopCount++;
    }
}